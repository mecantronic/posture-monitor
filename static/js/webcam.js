...
// Ocultar el elemento <video> al incio
...
function onResults(results) {
  ...
  if (frameCounter % 2 === 0) {
    // Actualizar cada 5 fotogramas (ajustable según sea necesario)
    ...
    if (goodTime > 0) {
      timeStringGood =
        "Good Posture Time : " + goodTime.toFixed(1).toString() + "s";
      canvasCtx.fillStyle = "green";
      canvasCtx.fillText(timeStringGood, 10, 250);
    } else {
      timeStringBad =
        "Bad Posture Time : " + badTime.toFixed(1).toString() + "s";
...
pose.setOptions({
  ...
  minTrackingConfidence: 0.9, // Minimum confidence value ([0.0, 1.0]) from the landmark-tracking model for the pose landmarks to be considered tracked successfully.  Setting it to a higher value can increase robustness of the solution, at the expense of a higher latency. Default to 0.5.
...
function findAngle(x1, y1, x2, y2) {
  ...
  var theta = Math.acos(
    ((y2 - y1) * -y1) /
      (Math.sqrt((x2 - x1) * (x2 - x1) + (y2 - y1) * (y2 - y1)) * y1)
  );
  var degree = Math.round((180 / Math.PI) * theta);
  return degree;
}

function decantProcess(
  lm,
  h,
  w,
  offsetThreshold = 100,
  ...
  // Left shoulder.
  frameMetrics.leftShoulder.x = parseInt(
    lm[POSE_LANDMARKS.LEFT_SHOULDER].x * w
  );
  frameMetrics.leftShoulder.y = parseInt(
    lm[POSE_LANDMARKS.LEFT_SHOULDER].y * h
  );

  // Right shoulder.
  frameMetrics.rightShoulder.x = parseInt(
    lm[POSE_LANDMARKS.RIGHT_SHOULDER].x * w
  );
  frameMetrics.rightShoulder.y = parseInt(
    lm[POSE_LANDMARKS.RIGHT_SHOULDER].y * h
  );
  ...
  // Calculate angles.
  const shoulderInclination = findAngle(
    frameMetrics.leftElbow.x,
  ...
  const elbowInclination = findAngle(
    frameMetrics.leftElbow.x,
    frameMetrics.leftElbow.y,
  ...
  canvasCtx.beginPath();
  canvasCtx.arc(
    frameMetrics.leftHip.x,
    frameMetrics.leftHip.y,
    7,
    0,
    2 * Math.PI
  );
  ...
  if (
    elbowInclination < elbowAngleThreshold[1] &&
    elbowInclination > elbowAngleThreshold[0]
  ) {
    badFrames = 0;
    goodFrames += 1;
    ...
    canvasCtx.fillText(
      `${parseInt(elbowInclination)}`,
      frameMetrics.leftElbow.x + 10,
      frameMetrics.leftElbow.y
    );
  ...
  if (
    shoulderInclination > 0 &&
    shoulderInclination < shoulderAngleThreshold[0]
  ) {
    badFrames = 0;
    goodFrames += 1;
    frameMetrics.goodPosture = true;
    ...
    canvasCtx.fillText(
      `${parseInt(shoulderInclination)}`,
      frameMetrics.leftShoulder.x + 10,
      frameMetrics.leftShoulder.y
    );
    ...
    shoulderInclination > shoulderAngleThreshold[0] &&
    shoulderInclination < shoulderAngleThreshold[1]
  ) {
    badFrames = 0;
    goodFrames += 1;
    frameMetrics.goodPosture = true;
